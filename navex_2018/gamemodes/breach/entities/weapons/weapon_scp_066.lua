...
SWEP.Primary.Sound			= "weapon/066/music066.wav"
...
function SWEP:Initialize()
	util.PrecacheSound("weapon/066/music066.wav")
	if CLIENT then
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		self.Owner:EmitSound( "attack066scp" )
		timer.Create( "Attack066", 0.4, 15, function()
			if !IsValid( self ) or !IsValid( self.Owner ) then
				timer.Destroy( "Attack066" )
				return
			end
...
function SWEP:Reload()
	...
	if (SERVER) then
		self.Owner:EmitSound( "navex/scp066/sound.wav", 75, 100, 0.3)
	end
end
