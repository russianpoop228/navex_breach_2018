...
function OpenClassMenu()
	...
	CLASSMENU:MakePopup()
	CLASSMENU.btnMinim:Hide()
	CLASSMENU.btnMaxim:Hide()
	...
	maininfo:SetText( "Менеджер классов" )
	maininfo:Dock( TOP )
	...
	sclass_name.Paint = function( self, w, h )
		...
		draw.Text( {
			text = "Скорость ходьбы: " .. math.Round(240 * selectedclass.walkspeed),
			font = "MTF_Third",
			xalign = TEXT_ALIGN_LEFT,
			yalign = TEXT_ALIGN_CENTER,
			pos = { 12, starpos + (atso) }
		...
		draw.Text( {
			text = "Скорость бега: " .. math.Round(240 * selectedclass.runspeed),
			font = "MTF_Third",
			xalign = TEXT_ALIGN_LEFT,
			yalign = TEXT_ALIGN_CENTER,
		...
		draw.Text( {
			text = "Дальность прыжка: " .. math.Round(200 * selectedclass.jumppower),
			font = "MTF_Third",
			xalign = TEXT_ALIGN_LEFT,
			yalign = TEXT_ALIGN_CENTER,
		...
		for i,cls in ipairs(v.roles) do
			...
			class_panel.Paint = function( self, w, h )
				if selectedclass == cls then
					draw.RoundedBox( 0, 0, 0, w, h, Color(v.color.r - 20, v.color.g - 20, v.color.b - 20, 200) )
				else
					draw.RoundedBox( 0, 0, 0, w, h, Color(v.color.r - 50, v.color.g - 50, v.color.b - 50, 200) )
				end
				draw.Text( {
